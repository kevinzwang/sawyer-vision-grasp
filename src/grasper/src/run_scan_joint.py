...
def collect_points():
    ...
    meshbb = mesh.get_minimal_oriented_bounding_box()

    tri_mesh = trimesh.Trimesh(np.asarray(mesh.vertices), np.asarray(mesh.triangles),
        vertex_normals=np.asarray(mesh.vertex_normals))
    
    tri_mesh_bb = trimesh.Trimesh(np.asarray(meshbb.vertices), np.asarray(meshbb.triangles),
        vertex_normals=np.asarray(meshbb.vertex_normals))

    vedo.show([tri_mesh], new=True)
    vedo.show([tri_mesh_bb], new=True)
    print("writing mesh")
    tri_mesh.export("mesh.obj")
    tri_mesh_bb.export("meshbb.obj")
